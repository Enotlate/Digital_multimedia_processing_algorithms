...
def main():
    # Задание №1
    img = cv2.imread(r'test.jpg', cv2.IMREAD_GRAYSCALE)  # Загрузка в оттенках серого для обработки
    img_color = cv2.imread(r'test.jpg')  # Загрузка цветного изображения для отображения

    standard_deviation = 9
    kernel_size = 11

    imgBlur_1 = AnotherGaussianBlur(img, kernel_size, standard_deviation)
    cv2.imshow('Original', img_color)  # Отображаем цветное изображение
